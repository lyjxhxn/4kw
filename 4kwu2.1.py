...
class Run():
    ...
    def down_ts(self,datas):
        if datas:
            self.jindu += 1 
            resopnse = requests.get(datas,stream = True)
            if resopnse.status_code == 200:
                path = 'E:/4K屋/temp/%s/%s'%(vod_name,juji)
                if not os.path.exists(path):
                    os.makedirs(path)
                for i in resopnse.iter_content(chunk_size=1024):
                    with open('E:/4K屋/temp/%s/%s/%s'%(vod_name,juji,datas.split('/')[-1]),'ab') as f: #ts完整路径进行切片，取最后一个值
                        f.write(i)
                        # f.close()
    ...
    def allok(self,ts_names):
        ...
        for ts_name in ts_names:
            ts_path = "E:\\4K屋\\temp\\%s\\%s\\%s"%(vod_name,juji,ts_name)
            with open("E:\\4K屋\\%s第%s集.ts"%(vod_name,juji),"ab+") as f:
                if os.path.exists(ts_path):
                    pass
                else:
                    print("%s文件丢失,尝试再次下载"%ts_name)
                    move = requests.get(ts_qianzhui + ts_name).content
